				...
				(function(ribbon) {
					function initRibbonBar() {
											...
											click : function(e, cmd) {
												...
												case "bottom-border":
													designer.actions
															.doAction(
																	...
																	spread,
																	{
																		lineborder : new Sheets.LineBorder(
																				"black",
																				1 /* thin */),
																		options : {
																			bottom : true
												...
												case "format-number":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Number.format);
													break;
												case "format-currency":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Currency.format);
													break;
												case "format-accouting":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Accounting.format);
													break;
												case "format-shortdate":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.ShortDate.format);
													break;
												case "format-longdate":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.LongDate.format);
													break;
												case "format-time":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Time.format);
													break;
												case "format-percentage":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Percentage.format);
													break;
												case "format-fraction":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Fraction.format);
													break;
												case "format-scientific":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Scientific.format);
													break;
												case "format-text":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Text.format);
													break;
												case "format-comma":
													designer.actions
															.doAction(
																	"setFormatter",
																	spread,
																	designer.res.commonFormats.Comma.format);
													break;
												case "format-more":
													if (formatCellDialog === undefined) {
														formatCellDialog = new designer.FormatDialog();
												...
												case "conditional-format":
													if (!conditionFormatPopup) {
														conditionFormatPopup = $(
																"#condition-format-popup")
																.gcuipopup(
																		{
																			autoHide : true,
																			position : {
													...
													break;
												case "insert-cells": {
													var selectionType = designer.spreadActions
															.getSelectionType(spread);
													switch (selectionType) {
													case 0 /* Sheet */
													:
														designer.MessageBox
																.show(
																		designer.res.insertCellInSheet,
																		designer.res.title,
																		2 /* warning */);
														break;
													case 2 /* OnlyColumn */
													:
														designer.spreadActions
													...
													:
														designer.MessageBox
																.show(
																		designer.res.insertCellInMixtureRange,
																		designer.res.title,
																		2 /* warning */);
														break;
													case 3 /* OnlyCells */
														...
														this._sheet
																.clear(
																		...
																		this._sheet
																				.getColumnCount(),
																		3 /* viewport */,
																		2 /* Style */| 1 /* Data */| 16 /* Sparkline */| 32 /* Axis */);
														break;
													case 2 /* OnlyColumn */
													:
														designer.spreadActions
												...
												case "group":
													var _sheet = spread
															.getActiveSheet();
													var selections = _sheet
															.getSelections();
													if (selections.length > 1) {
														designer.MessageBox
																.show(
																		designer.res.NotExecInMultiRanges,
																		designer.res.title,
																		2 /* warning */);
														return;
													}
												...
												case "template-design-mode":
													var activeSheet = spread
															.getActiveSheet();
													var designMode = $("#template-design-mode");
													var designModeText = designMode
															.find("span.ui-button-text");
													var checkedStatus = designMode
															.data("checked");
													...
													if (checkedStatus) {
														...
														_replacePaint();
														designModeText
																.addClass("template-design-mode-checked");
												...
												case "name-manager":
													...
													break;
												case "new-conversionage":
													var sheet = new GC.Spread.Sheets.Worksheet('新建换算页');
													spread.addSheet(spread.getSheetCount(),sheet);
													spread.setActiveSheetIndex(spread.getSheetCount() - 1)

													break;
												case "get-item": //获取预算科目
													var mynamespace = {};
													var spread = GC.Spread.Sheets.findControl(document.getElementById('ss'));
										            var ns = GC.Spread.Sheets;
										            var treeCellNodeType = Tools.initSpreadTreeNode(ns);
										            var year = localStorage.getItem('year');
										            Tools.getItem(spread, ns, year, treeCellNodeType);
													break;
												case "syswinClearTplCache":
													//if (window.SyswinVal.clearTplCache()) {
														window.location.reload();
													//}
													break;

												//“上传”ssjson和excel
												case "saveSpreadExcel":
													var excelIo = new GC.Spread.Excel.IO();
													var templateId = request("templateId");		//模板id
													var templateName = request("templateName"); //模板名称
													if (typeof templateId === 'undefined' || templateId.length <= 0) {
														designer.MessageBox.show("获取模板名称失败，请重试!", "提示", 1 /* error */, 0/* okCancel */,null);
													}
													var password = "";
													...
													excelIo.save(json, function (blob) {
														...
														$.ajax({
															method:"POST",
															url : getRootPath() + '/sys_file/upload_txt_and_excel',//'/sys_file/upload_json',
															data: fd,
															async : false,
															processData: false,
													        contentType: false
															...
															if (result.isSuccess) {
																// TODO: 服务器处理上传文件是异步的，返加操作成功并不是100%的上传成功
																top.layer.msg("操作成功", {time: 3000, icon: 1});
													        	//parent.refresh();
																//parent.layer.close(parent.layer.getFrameIndex(window.name));
															} else {
																designer.MessageBox.show("保存失败，请重试!", "提示", 1 /* error */, 0/* okCancel */,null);
															}
														}).fail(function (e) {
															//console.log(e);
															designer.MessageBox.show("请求失败，请重试!", "提示", 1 /* error */, 0/* okCancel */,null);
														});
													});
													
													break;
												case "new-accesspage":	//新建数页
													if (!newAccesspageDialog) {
														newAccesspageDialog = new designer.newAccesspageDialog();
													}
													newAccesspageDialog.open();
													break;
												...
												case "sparkline-high-point":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	[
																			getCheckBoxIconStatus("sparkline-high-point"),
																			"showHigh" ]);
													break;
												case "sparkline-low-point":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	[
																			getCheckBoxIconStatus("sparkline-low-point"),
																			"showLow" ]);
													break;
												case "sparkline-first-point":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	[
																			getCheckBoxIconStatus("sparkline-first-point"),
																			"showFirst" ]);
													break;
												case "sparkline-last-point":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	[
																			getCheckBoxIconStatus("sparkline-last-point"),
																			"showLast" ]);
													break;
												case "sparkline-negative-point":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	[
																			getCheckBoxIconStatus("sparkline-negative-point"),
																			"showNegative" ]);
													break;
												case "sparkline-marker-point":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	[
																			getCheckBoxIconStatus("sparkline-marker-point"),
																			"showMarkers" ]);
													break;
												...
												case "table-header-row":
													...
													designer.actions
															.doAction(
																	"setTableHeaderRow",
																	spread,
																	getCheckBoxIconStatus("table-header-row"));
													break;
												case "table-total-row":
													...
													designer.actions
															.doAction(
																	"setTableTotalRow",
																	spread,
																	getCheckBoxIconStatus("table-total-row"));
													break;
												case "table-banded-rows":
													...
													designer.actions
															.doAction(
																	"setTableBandedRows",
																	spread,
																	getCheckBoxIconStatus("table-banded-rows"));
													break;
												case "table-first-column":
													...
													designer.actions
															.doAction(
																	"setTableFirstColumn",
																	spread,
																	getCheckBoxIconStatus("table-first-column"));
													break;
												case "table-last-column":
													...
													designer.actions
															.doAction(
																	"setTableLastColumn",
																	spread,
																	getCheckBoxIconStatus("table-last-column"));
													break;
												case "table-banded-columns":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	getCheckBoxIconStatus("table-banded-columns"));
													break;
												case "table-filter-button":
													...
													designer.actions
															.doAction(
																	...
																	spread,
																	getCheckBoxIconStatus("table-filter-button"));
													break;
												case "table-styles":
													if (!formatTablePopup) {
														_initTableFormat();
																...
																.gcuipopup(
																		{
																			...
																			showing : function(
																					...
																					args) {
																				$(
																						"#format-table-popup span.container-span")
																						.removeClass(
																								"table-format-item-selected");
																				if (_selectedTableStyleName) {
																					$(
																							"#format-table-popup span.table-format-preview."
																									+ _selectedTableStyleName)
																							.parent()
																							.addClass(
																									"table-format-item-selected");
																				}
												...
												case "formulaSparkline-setting":
													...
													var col = sheet
															.getActiveColumnIndex();
													var expr = designer.util
															.parseFormulaSparkline(
																	row, col);
													if (!expr) {
														break;
													}
													var formulaSparkline = designer.wrapper.spread
															.getSparklineEx(expr.functionName);
													var option = {
														row : row,
														col : col
													};
																...
																.gcuipopup(
																		{
																			...
																			showing : function(
																				...
																				// item
																				$(
																						"#format-slicer-popup span.container-span")
																						.removeClass(
																								"slicer-format-item-selected");
																				var selectedSlicers = designer.util
																						.getSelectedSlicers(sheet);
																				var styleName = selectedSlicers
																						&& selectedSlicers[0]
																						&& selectedSlicers[0]
																								.style()
																						&& selectedSlicers[0]
																				...
																				if (styleName) {
																					...
																					// shortname
																					styleName = styleName
																							.substr("slicerstyle".length);
																					$(
																							"#format-slicer-popup span.slicer-format-"
																									+ styleName)
																							.parent()
																							.addClass(
																									"slicer-format-item-selected");
															...
															var customPrefix = name
																	.substring(
																			...
																			name
																					.indexOf('-'));
															$(
																	'.custom-format-slicer')
																	.css(
																			'display',
																			'block');
															var spanClass = 'slicer-format-'
																	+ customPrefix;
															$('.' + spanClass)
																	.css(
																			'display',
													...
													setTimeout(
															function() {
																$(
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-paste");
																$(
																		"#paste-options")
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.home.paste)
																		.append(
																				"<span class=\"ui-custom-triangle2\"></span>");
															}, 0);
													break;
												case "freeze-panes":
												case "freeze-toprow":
													...
													setTimeout(
															function() {
																$(
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-freeze");
																$(
																		"#freeze-panes")
																		.find(
																				".ui-button-text")
																		.html(
																				designer.res.ribbon.view.freezePane)
																		.append(
																				"<span class=\"ui-custom-triangle\"></span>");
															}, 0);
													break;
												case "format-more":
													setTimeout(
															function() {
																$(
																		...
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.home.general);
															}, 0);
													break;
												case "set-rowheight":
												case "set-columnwidth":
													...
													setTimeout(
															function() {
																$(
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-format");
																$(
																		"#format-row-column")
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.home.format)
																		.append(
																				"<span class=\"ui-custom-triangle2\"></span>");
															}, 0);
													break;
													...
													setTimeout(
															function() {
																$("#find-goto")
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-find");
																$("#find-goto")
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.home.find)
																		.append(
																				"<span class=\"ui-custom-triangle\"></span>");
															}, 0);
													break;
													...
													setTimeout(
															function() {
																$(
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-sortfilter");
																$(
																		"#sort-filter")
																		.find(
																				".ui-button-text")
																		.html(
																				designer.res.ribbon.home.sortFilter)
																		.append(
																				"<span class=\"ui-custom-triangle\"></span>");
															}, 0);
													break;
													...
													setTimeout(
															function() {
																$(
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-insert");
																$(
																		"#insert-row-column")
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.home.insert)
																		.append(
																				"<span class=\"ui-custom-triangle2\"></span>");
															}, 0);
													break;
													...
													setTimeout(
															function() {
																$(
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-delete");
																$(
																		"#delete-row-column")
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.home.Delete)
																		.append(
																				"<span class=\"ui-custom-triangle2\"></span>");
															}, 0);
													break;
															...
															function() {
																$(
																		...
																		.find(
																				".ui-button-text")
																		.text(
																				designer.res.ribbon.sparkLineDesign.sparklineWeight);
															}, 0);
													break;
												case "bottom-border":
												case "top-border":
													...
													setTimeout(
															function() {
																$("#borders")
																		...
																		.removeClass()
																		.addClass(
																				"ui-button-icon-primary ui-icon gcui-ribbon-bottomborder");
															}, 0);
													break;
												}
												switch (cmd.commandName) {
						...
						var showSubPopupMenu = function(popup, button, delay) {
							if (showingPopup && showingPopup.is(popup)
									&& showingPopup.gcuipopup('isVisible')) {
								return;
							}
							if (showingPopup) {
							...
							if (delay === undefined) {
								var ticket = setTimeout(function() {
									...
									showSubItemTicket.splice(showSubItemTicket
											.indexOf(ticket), 1);
									showingPopup = popup;
								}, 0);
						...
						var hideSubPopupMenu = function(popup, delay) {
							if (delay === undefined) {
								var ticket = setTimeout(function() {
									...
									hideSubItemTicket.splice(hideSubItemTicket
											.indexOf(ticket), 1);
						...
						$("#condition-format-popup button")
								.on(
										"mouseenter",
										function(e) {
											var el = e.currentTarget;

											var hoverElement = $(el).children(
													"span.ui-button-text-big");
											if (hoverElement.length === 0) {
												hoverElement = $(el)
														.children(
																"span.ui-button-text-small");
												if (hoverElement.length === 0) {
													hoverElement = $(el)
															.children(
																	"span.icon-button-container");
												}
											}
											$(hoverElement).addClass(
													"ui-state-hover");
											...
											case "highlight-cells-rules":
												if (!highlightCellsRulesPopup) {
													highlightCellsRulesPopup = createSubMenuItem(
															$("#highlight-cells-rules-popup"),
															$(el));
												}
												showSubPopupMenu(
														highlightCellsRulesPopup,
														$(el), delayTime);
												break;
											case "top-bottom-rules":
												if (!topBottomRulesPopup) {
													topBottomRulesPopup = createSubMenuItem(
															$("#top-bottom-rules-popup"),
															$(el));
												}
												showSubPopupMenu(
														topBottomRulesPopup,
											...
											case "data-bars":
												if (!dataBarsPopup) {
													dataBarsPopup = createSubMenuItem(
															$("#data-bars-popup"),
															$(el));
												}
												showSubPopupMenu(dataBarsPopup,
														$(el), delayTime);
												break;
											case "color-scales":
												if (!colorScalesPopup) {
													colorScalesPopup = createSubMenuItem(
															$("#color-scales-popup"),
															$(el));
												}
												showSubPopupMenu(
											...
											case "icon-sets":
												if (!iconSetsPopup) {
													iconSetsPopup = createSubMenuItem(
															$("#icon-sets-popup"),
															$(el));
												}
												showSubPopupMenu(iconSetsPopup,
														$(el), delayTime);
												break;
											case "clear-rules":
												if (!clearRulesPopup) {
													clearRulesPopup = createSubMenuItem(
															$("#clear-rules-popup"),
															$(el));
												}
												showSubPopupMenu(
						...
						var closeConditionalFormatMenu = function() {
							...
							$("#condition-format-popup").children("ul")
									.addClass("hidden");
						};

						var addDataBarAutoRule = function(color, isGradient) {
							var rule;
							var sheet = designer.wrapper.spread
									.getActiveSheet();
							...
							for (var i = formats.count() - 1; i >= 0; i--) {
								...
								if (iRule.ruleType() === Sheets.ConditionalFormatting.RuleType.dataBarRule) {
									if (JSON.stringify(iRule.ranges()) === JSON
											.stringify(rule.ranges())) {
										formats.removeRule(iRule);
										break;
									}
								}
							}
							designer.actions.doAction("addRule",
									designer.wrapper.spread, rule);
						};

						var addColorScaleAutoRule = function(color1, color2,
								color3) {
							var rule;
							var sheet = designer.wrapper.spread
									.getActiveSheet();
							...
							if (color3) {
								...
								minColor = color3;
								rule = new Sheets.ConditionalFormatting.ScaleRule(
										11 /* ThreeScaleRule */, minType,
										minValue, minColor, midType, midValue,
										midColor, maxType, maxValue, maxColor,
										ranges);
							} else {
								rule = new Sheets.ConditionalFormatting.ScaleRule(
										10 /* TwoScaleRule */, minType,
										minValue, minColor, null, null, null,
										maxType, maxValue, maxColor, ranges);
							}

							var formats = sheet.conditionalFormats;
							for (var i = formats.count() - 1; i >= 0; i--) {
								var iRule = formats.getRule(i);
								if (iRule.ruleType() === Sheets.ConditionalFormatting.RuleType.threeScaleRule
										&& rule.ruleType() === Sheets.ConditionalFormatting.RuleType.threeScaleRule
										|| iRule.ruleType() === Sheets.ConditionalFormatting.RuleType.twoScaleRule
										&& rule.ruleType() === Sheets.ConditionalFormatting.RuleType.twoScaleRule) {
									if (JSON.stringify(iRule.ranges()) === JSON
											.stringify(rule.ranges())) {
										formats.removeRule(iRule);
										break;
						...
						var addIconSetAutoRule = function(iconSetType) {
							var sheet = designer.wrapper.spread
									.getActiveSheet();
							var ranges = sheet.getSelections();
							var rule = new Sheets.ConditionalFormatting.IconSetRule(
									iconSetType, ranges);
							var formats = sheet.conditionalFormats;
							for (var i = formats.count() - 1; i >= 0; i--) {
								var iRule = formats.getRule(i);
								...
								.on(
										...
										function(e) {
											...
											case "highlight-cells-rules-a-date-occurring":
												dateOccurringFormatDialog
														.open();
												closeConditionalFormatMenu();
												break;
											case "highlight-cells-rules-duplicate-values":
												duplicateValuesFormatDialog
														.open();
												closeConditionalFormatMenu();
												break;
											...
											case "gradient-fill-orange-data-bar":
												addDataBarAutoRule("orange",
														true);
												closeConditionalFormatMenu();
												break;
											case "gradient-fill-lightblue-data-bar":
												addDataBarAutoRule("lightblue",
														true);
												closeConditionalFormatMenu();
												break;
											case "gradient-fill-purple-data-bar":
												addDataBarAutoRule("purple",
														true);
												closeConditionalFormatMenu();
												break;
											case "solid-fill-blue-data-bar":
												addDataBarAutoRule("blue",
														false);
												closeConditionalFormatMenu();
												break;
											...
											case "gyr-color-scale":
												addColorScaleAutoRule("green",
														"yellow", "red");
												closeConditionalFormatMenu();
												break;
											case "ryg-color-scale":
												addColorScaleAutoRule("red",
														"yellow", "green");
												closeConditionalFormatMenu();
												break;
						...
						// Set special class
						// For now, I fixed some ribbon layout bugs by this
						// function,
						// If Ribbon team fix them in the future, I will remove
						// this function.
						setSpecialClass();
					...
					function toggleCheckBoxIcon(id) {
						$("#" + id).find(".ui-icon").toggleClass(
								"gcui-ribbon-checked").toggleClass(
								"gcui-ribbon-unchecked");
					}

					// Get if a element is checked
					function getCheckBoxIconStatus(id) {
						return $("#" + id).find(".ui-icon").hasClass(
								"gcui-ribbon-checked");
					}

					function setCheckBoxIconStatus(id, checked) {
						var ele = $("#" + id).find(".ui-icon").removeClass(
								"gcui-ribbon-checked").removeClass(
								"gcui-ribbon-unchecked");
						if (checked) {
					...
					function increasedFontSize() {
						var sizeArray = [ 8, 9, 10, 11, 12, 14, 16, 18, 20, 24,
								26, 28, 36, 48, 72 ];
					...
					// For now, I fixed some ribbon layout bugs by this
					// function,
					// If Ribbon team fix them in the future, I will remove this
					// function.
					function setSpecialClass() {
						...
						$("#paste-options")
								...
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-paste");
						$("#paste-options").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle2\"></span>");
						$("#freeze-panes")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-freeze");
						$("#freeze-panes").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle\"></span>");
						$("#find-goto")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-find");
						$("#find-goto").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle\"></span>");
						$("#sort-filter")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-sortfilter");
						$("#sort-filter").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle\"></span>");
						$("#condition-format")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-conditionalformat");
						$("#insert-row-column")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-insert");
						$("#insert-row-column").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle2\"></span>");
						$("#delete-row-column")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-delete");
						$("#delete-row-column").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle2\"></span>");
						$("#format-row-column")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-format");
						$("#format-row-column").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle2\"></span>");
						$("#calculate-option")
								.find(".ui-icon")
								.removeClass()
								.addClass(
										"ui-button-icon-primary ui-icon gcui-ribbon-calculateoption");
						$("#calculate-option").find(".ui-button-text").append(
								"<span class=\"ui-custom-triangle\"></span>");
						$("#borders")
								.find(".ui-icon")
								.removeClass()
						...
						$("#showhide-vgridline").parents(".gcui-ribbon-list")
								.addClass("gcui-ribbon-marginright");
						$("#showhide-columnheader")
								.parents(".gcui-ribbon-list").addClass(
										"gcui-ribbon-marginright");
					}

					function hideSparklineTab() {
						if ($("a[href='#sparklineTab']").is(':visible')) {
							$(".ribbon-bar").gcuiribbon("setTabPageVisible",
									"sparklineTab", false);
						}
					}
					...
					function showFormulaSparklineTab() {
						$(".ribbon-bar").gcuiribbon("setTabPageVisible",
								"formulaSparklineTab", true);
						$(".ribbon-bar").gcuitabs("select",
								"formulaSparklineTab");
					}
					...
					function updateCellStyle() {
						...
						var style = sheet.getActualStyle(sheet
								.getActiveRowIndex(), sheet
								.getActiveColumnIndex());
						...
						if (style && style.font) {
							...
							if (font.fontWeight !== 'normal') {
								$('.ribbon-bar').gcuiribbon('setButtonChecked',
										'font-weight', true);
							}
							...
							if (font.fontSize) {
								setDropDownContent('#font-size', font.fontSize
										.substring(0, font.fontSize.length - 2));
							}
							if (font.fontFamily) {
								...
								for ( var fontKey in designer.res.ribbon.fontFamilies) {
									if (regex
											.test(designer.res.ribbon.fontFamilies[fontKey].name)) {
							...
							var font = designer.util
									.parseFont(designer.res.defaultFont);
							setDropDownContent('#font-size', font.fontSize
									.substring(0, font.fontSize.length - 2));
					...
					function updateFormat() {
						...
						if (!style || !style.formatter
								|| style.formatter === 'general') {
							setDropDownContent('#number-format',
									designer.res.generalFormat);
						} else {
					...
					function updateSparkline() {
						...
						var expr;
						if (sheet.getSparkline(sheet.getActiveRowIndex(), sheet
								.getActiveColumnIndex()) instanceof Sheets.Sparklines.Sparkline) {
							// Show sparkline tab
							showSparklineTab();

							// Update sparkline type
							var sparkline = sheet.getSparkline(sheet
									.getActiveRowIndex(), sheet
									.getActiveColumnIndex());
							$('.ribbon-bar').gcuiribbon("setButtonChecked",
									"sparkline-type-line", false);
							$('.ribbon-bar').gcuiribbon("setButtonChecked",
									"sparkline-type-column", false);
							$('.ribbon-bar').gcuiribbon("setButtonChecked",
									"sparkline-type-winloss", false);
							switch (sparkline.sparklineType()) {
							case 0 /* line */
							:
								$('.ribbon-bar').gcuiribbon("setButtonChecked",
										"sparkline-type-line", true);
								break;
							case 1 /* column */
							:
								$('.ribbon-bar').gcuiribbon("setButtonChecked",
										"sparkline-type-column", true);
								break;
							case 2 /* winloss */
							:
								$('.ribbon-bar').gcuiribbon("setButtonChecked",
										"sparkline-type-winloss", true);
								break;
							}

							// Update sparkline point
							var sparklineSetting = sparkline.setting();
							setCheckBoxIconStatus("sparkline-high-point",
									sparklineSetting.options.showHigh);
							setCheckBoxIconStatus("sparkline-low-point",
									sparklineSetting.options.showLow);
							setCheckBoxIconStatus("sparkline-first-point",
									sparklineSetting.options.showFirst);
							setCheckBoxIconStatus("sparkline-last-point",
									sparklineSetting.options.showLast);
							setCheckBoxIconStatus("sparkline-negative-point",
									sparklineSetting.options.showNegative);
							setCheckBoxIconStatus("sparkline-marker-point",
									sparklineSetting.options.showMarkers);
						} else if (expr = designer.util.parseFormulaSparkline(
								row, col)) {
					...
					function updateTable() {
						var sheet = designer.wrapper.spread.getActiveSheet();
						if (sheet.tables.find(sheet.getActiveRowIndex(), sheet
								.getActiveColumnIndex()) instanceof Sheets.Tables.Table) {
							// show table tab
							showTableTab();

							// disable insert table item
							$(".ribbon-bar").gcuiribbon("setButtonDisabled",
									"insert-table", true);

							// update table tab
							var table = sheet.tables.find(sheet
									.getActiveRowIndex(), sheet
									.getActiveColumnIndex());
							setCheckBoxIconStatus("table-header-row", table
									.showHeader());
							setCheckBoxIconStatus("table-total-row", table
									.showFooter());
							setCheckBoxIconStatus("table-banded-rows", table
									.bandRows());
							setCheckBoxIconStatus("table-first-column", table
									.highlightFirstColumn());
							setCheckBoxIconStatus("table-last-column", table
									.highlightLastColumn());
							setCheckBoxIconStatus("table-banded-columns", table
									.bandColumns());
							setCheckBoxIconStatus("table-filter-button", table
									.filterButtonVisible());
							$(".designer-table-name").val(table.name());

							// save table style' name which can be used to mark
							// corresponding style item in table style popup
							var style = table.style(), styleName = style.name();
							_selectedTableStyleName = (styleName && styleName
									.toLowerCase())
									|| "";
						} else {
							hideTableTab();
					...
					function updateShowHide() {
						var sheet = designer.wrapper.spread.getActiveSheet();
						setCheckBoxIconStatus("showhide-columnheader",
								sheet.options.colHeaderVisible);
						setCheckBoxIconStatus("showhide-rowheader",
								sheet.options.rowHeaderVisible);
						setCheckBoxIconStatus("showhide-hgridline",
								sheet.options.gridline.showHorizontalGridline);
						setCheckBoxIconStatus("showhide-vgridline",
								sheet.options.gridline.showVerticalGridline);
						setCheckBoxIconStatus("showhide-tabstrip",
								designer.wrapper.spread.options.tabStripVisible);
						setCheckBoxIconStatus("showhide-newtab",
								designer.wrapper.spread.options.newTabVisible);
					}
					...
					function updateProtectLock() {
						...
						if (sheet.options.isProtected) {
							$(".ribbon-bar").find("#protect-sheet").addClass(
									"hidden");
							$(".ribbon-bar").find("#unprotect-sheet")
									.removeClass("hidden");
						} else {
							$(".ribbon-bar").find("#protect-sheet")
									.removeClass("hidden");
							$(".ribbon-bar").find("#unprotect-sheet").addClass(
									"hidden");
						}
						if (sheet.getCell(sheet.getActiveRowIndex(),
								sheet.getActiveColumnIndex()).locked()) {
							$(".ribbon-bar").find("#lock-cells").addClass(
									"hidden");
							$(".ribbon-bar").find("#unlock-cells").removeClass(
									"hidden");
						} else {
							$(".ribbon-bar").find("#lock-cells").removeClass(
									"hidden");
							$(".ribbon-bar").find("#unlock-cells").addClass(
									"hidden");
						}
					}
					...
					function updateZoomToStatusBar() {
						var sheet = designer.wrapper.spread.getActiveSheet();
						$(".statusBar").find(".statusbar-zoom-value").text(
					...
					function exitTemplateDesignMode() {
						...
						designModeText
								.removeClass("template-design-mode-checked");
					...
					function _createCustomCellStyle() {
						...
						_cellStyleLayout(
								...
								$(".custom-cell-styles-preview"), true);
					...
					function resetSpreadNamedStyle() {
						...
						_addBuildInNamedStyle(goodBadStyle, dataAndModelStyle,
								headingsStyles, themedCellStyles,
								numberFormatStyles);
						...
						if (_isEmptyObject(designer.CellStyleDialog.existedCustomCellStyle)) {
							...
							$(".custom-cell-styles-preview").parent().addClass(
									"hidden");
						}

						_cellStyleLayout(
								designer.res.cellStylesDialog.goodBadAndNeutralContent,
								$(".good-bad-style-preview"), false,
								goodBadStyle);
						_cellStyleLayout(
								designer.res.cellStylesDialog.dataAndModelContent,
								$(".data-model-style-preview"), false,
								dataAndModelStyle);
						_cellStyleLayout(
								designer.res.cellStylesDialog.titlesAndHeadingsContent,
								$(".titles-headings-preview"), false,
								headingsStyles);
						_cellStyleLayout(
								designer.res.cellStylesDialog.themedCellStyleContent,
								$(".themed-preview"), false, themedCellStyles);
						_cellStyleLayout(
								designer.res.cellStylesDialog.numberFormatContent,
								$(".number-format-preview"), false,
								numberFormatStyles);
					...
					function _buildInGoodBadStyle(spread) {
						...
						for (var i = 0; i < nameSet.length; i++) {
							...
							if (namedStyle) {
								...
								switch (name) {
								case "normal":
									goodBadAndNeutralStyles["normal"] = {
										name : "Normal"
									};
									break;
								case "bad":
					...
					function _buildInDataAndModelStyle(spread) {
						var nameSet = [ "calculation", "check cell",
								"explanatory text", "input", "linked cell",
								"note", "output", "warning text" ];
					...
					function _buildInTitlesAndHeadingsStyles(spread) {
						var nameSet = [ "heading 1", "heading 2", "heading 3",
								"heading 4", "title", "total" ];
						...
						for ( var item in themes) {
							...
							if (namedStyle) {
								...
								if (colorDetails.length === 1) {
									var num = colorDetails[0]
											.substr(colorDetails[0].length - 1);
									themeColor = "Accent " + num + " 0";
									fontColor = "white";
								} else if (colorDetails.length === 2) {
									var num = colorDetails[1]
											.substr(colorDetails[1].length - 1);
									var colorDarker = 100 - parseInt(colorDetails[0]);
									themeColor = "Accent " + num + " "
											+ colorDarker + "%";
									if (colorDarker <= 50) {
										fontColor = "white";
									} else {
								...
								themeColor = designer.ColorHelper.parse(
										themeColor, designer.wrapper.spread
												.getActiveSheet()
												.currentTheme().colors());
								themedCellStyles[key] = {
									name : item,
									backColor : themeColor.color,
									foreColor : fontColor
								};
							}
						}
					...
					function _buildInNumberFormatStyles(spread) {
						var nameSet = [ "comma", "comma [0]", "currency",
								"currency [0]", "percent" ];
					...
					function _parseCssBorderStyle(borderStyle) {
						...
						var lineColor = "black";
						if (styles.length === 3) {
							if (styles[0] === "thin" || styles[0] === "medium"
									|| styles[0] === "thick") {
					...
					function _convertToCSSStyle(cssStyle, spreadStyleObject) {
						...
						for ( var prop in spreadStyleObject) {
							if ((prop === "backColor" || prop === "foreColor")
									&& spreadStyleObject[prop]) {
								var colorItem = designer.ColorHelper.parse(
										spreadStyleObject[prop],
										designer.wrapper.spread
												.getActiveSheet()
												.currentTheme().colors());
						...
						if (cssStyle.font && cssStyle.themeFont) {
							cssStyle.font = composeFont(cssStyle.font,
									designer.util
											.getThemeFont(cssStyle.themeFont));
						}
						...
						if (cssStyle.textDecoration) {
							cssStyle["textDecoration"] = designer.util
									.toCSSTextDecoration(cssStyle.textDecoration);
						}
					}
					...
					function _getCSSBorder(border) {
						...
						var cssborder = [];
						var borderStyle = [ "dotted", "solid", "double",
								"dashed" ];
						var lineName;
						if (border.style) {
						...
						if (border.color) {
							var colorItem = designer.ColorHelper.parse(
									border.color, designer.wrapper.spread
											.getActiveSheet().currentTheme()
											.colors());
							cssborder[2] = colorItem.color;
						} else {
							var colorItem = designer.ColorHelper.parse(
									"Text 1", designer.wrapper.spread
											.getActiveSheet().currentTheme()
											.colors());
							cssborder[2] = colorItem.color;
						}
					...
					function _cellStyleLayout(data, parent, isCustomCellStyle,
							styles) {
						if (isCustomCellStyle) {
							if (!_isEmptyObject(data)) {
								$(".cell-style-custom-li")
										.removeClass("hidden");
								$(".custom-cell-styles-preview").parent()
										.removeClass("hidden");
								for ( var item in data) {
									...
									var div = $("<div></div>").text(
											customStyleName).addClass(
											"custom-cell-style-symbol")
											.appendTo(outerSpan);
									outerSpan.appendTo(parent);
									var cssStyle = {};
									_convertToCSSStyle(cssStyle, data[item]);
									_setPreivewStyle(cssStyle, div);
									_addNamedStyle(customStyleName,
											designer.wrapper.spread); // Keep
																		// custom
																		// cellstyle
																		// when
																		// user
							...
							for ( var item in data) {
								...
								var div = $("<div></div>").text(data[item])
										.addClass("cell-style-symbol")
										.appendTo(outerSpan);
								outerSpan.appendTo(parent);
								if (styles) {
					...
					function _setPreivewStyle(style, target) {
						if (style) {
							...
							if (style.font) {
								if (style.themeFont) {
									style.font = composeFont(
											style.font,
											designer.util
													.getThemeFont(style.themeFont));
								}
								target.css("font", style.font);
							}
							if (style.borderLeft) {
								target.css("border-left",
										_getCSSBorder(style.borderLeft));
							}
							if (style.borderRight) {
								target.css("border-right",
										_getCSSBorder(style.borderRight));
							}
							if (style.borderBottom) {
								target.css("border-bottom",
										_getCSSBorder(style.borderBottom));
							}
							if (style.borderTop) {
								target.css("border-top",
										_getCSSBorder(style.borderTop));
							}
							if (style.textAlign) {
								...
								if (!isNaN(style.textDecoration)) {
									target
											.css(
													"text-decoration",
													designer.util
															.toCSSTextDecoration(style.textDecoration));
								} else {
					...
					var cellStyleSelectItem = function(evt, ui) {
						...
						case "cellstyle-modify":
							...
							_addNamedStyle(selectedStyleName,
									designer.wrapper.spread);
							modifyStyle = cellStyleSpread
									.getNamedStyle(selectedStyleName);
							cellStylefFormatDialog.open("numbers", undefined,
									modifyStyle, true); // for format dialog
														// open, rewrite the
														// setting.
							cellStylefFormatDialog.setFormatDirectly(false); // don't
																				// set
																				// style
																				// for
																				// spread
																				...
																				// okClicked
																				// event.
							$(cellStylefFormatDialog)
									.on(
											'okClicked',
											function(evt, args) {
												if (designer.CellStyleDialog.existedCustomCellStyle
														.hasOwnProperty(selectedStyleName)) {
													$
															.extend(
																	designer.CellStyleDialog.existedCustomCellStyle[selectedStyleName],
																	args);
												} else if (designer.CellStyleDialog.buildInCellStyle
														.hasOwnProperty(selectedStyleName)) {
													$
															.extend(
																	designer.CellStyleDialog.buildInCellStyle[selectedStyleName],
																	args);
												}
												...
												var cssStyle = {};
												_convertToCSSStyle(cssStyle,
														args);
												_setPreivewStyle(cssStyle, $(
														selectedParent)
														.children());
						...
						case "cellstyle-delete":
							...
							if (cellStyleSheet.getNamedStyle(selectedStyleName)) {
								cellStyleSheet
										.removeNamedStyle(selectedStyleName);
							}
							if (cellStyleSpread
									.getNamedStyle(selectedStyleName)) {
								cellStyleSpread
					...
					function cellStyleClick(e) {
						...
						designer.actions.doAction("setStyleByName",
								designer.wrapper.spread, styleName);
						$("#cell-styles-popup").gcuipopup("hide");
					}
					...
					function _addNamedStyle(name, spread) {
						...
						if (!spread.getNamedStyle(name)) {
							if (designer.CellStyleDialog.buildInCellStyle
									.hasOwnProperty(key)) {
								designer.actions
										.doAction(
												"addNamedStyle",
												spread,
												designer.CellStyleDialog.buildInCellStyle[key]);
							} else if (designer.CellStyleDialog.existedCustomCellStyle
									.hasOwnProperty(key)) {
								designer.actions
										.doAction(
												"addNamedStyle",
												spread,
												designer.CellStyleDialog.existedCustomCellStyle[key]);
							}
						}
					}
					...
					function cellStyleContextMenu(e) {
						...
						var sheet = designer.wrapper.spread.getActiveSheet();
						selectedStyleName = $(selectedParent).attr("data-name")
								.toLowerCase();
					...
					function _attchCellStyleEvent() {
						...
						$(".cellstyle-menu").menu({
							select : cellStyleSelectItem
						});
						$(".cell-style-symbol").bind("mouseenter",
								cellStyleMouseEnter);
						$(".cell-style-symbol").bind("mouseout",
								cellStyleMouseOut);
						$(".cell-style-symbol").bind("click", cellStyleClick);
						$(".cell-style-symbol").bind("contextmenu",
								cellStyleContextMenu);

						$(".new-cell-style-text, .new-cell-style-button").bind(
								"mouseout",
								function() {
									$(".new-cell-style-text").removeClass(
											"ui-state-hover");
								});
						$(".new-cell-style-text, .new-cell-style-button").bind(
								"mouseenter",
								function() {
									$(".new-cell-style-text").addClass(
											"ui-state-hover");
								});
					...
					function _reAttachCustomCellStyleEvent() {
						...
						$(".custom-cell-style-symbol").bind("mouseenter",
								cellStyleMouseEnter);
						$(".custom-cell-style-symbol").bind("mouseout",
								cellStyleMouseOut);
						$(".custom-cell-style-symbol").bind("click",
								cellStyleClick);
						$(".custom-cell-style-symbol").bind("contextmenu",
								cellStyleContextMenu);
					}
					...
					function _initTableFormat() {
						...
						_createTableFormat('custom-preview', 'custom',
								customCount);
						_createTableFormat('light-preview', 'light', lightCount);
						_createTableFormat('medium-preview', 'medium',
								mediumCount);
						_createTableFormat('dark-preview', 'dark', darkCount);

						$('.table-format-preview, .table-format-preview-button')
								.mouseenter(
										function(e) {
											var o = (e.srcElement || e.target);
											$(o).closest("button").find(
													"span.container-span")
						...
						$('.table-format-preview-button')
								.mouseup(
										function(e) {
											var spread = designer.wrapper.spread;
											var o = e.srcElement || e.target;
											var ele = ($(o).closest("button")
													.find("span span")[0]);
											...
											if (style.indexOf('custom') === -1) {
												designer.actions
														.doAction(
																"formatAsTable",
																spread,
																Sheets.Tables.TableThemes[style]);
												updateTable();
											} else {
					...
					function _addCustomIConTip(name, position) {
						...
						var formatClass = tableFormatClassPrefix + formatType
								+ position;
						var tableStyleName = name
								.substring(name.indexOf('-') + 1);
						var customPrefix = name.substring(0, name.indexOf('-'));
						$('.' + formatClass).attr('title', tableStyleName);
					}

					function _createTableFormat(previewClass, formatType,
							formatTypeNumber) {
						var tableFormatClassPrefix = 'table-format-';
						var commonClass = 'table-format-preview';
						var formatSpan;
						for (var i = 1; i <= formatTypeNumber; i++) {
							var formatClass = tableFormatClassPrefix
									+ formatType + i;
							if (formatType === 'custom') {
								formatSpan = $('<span></span>').addClass(
										formatClass + ' ' + commonClass + ' '
												+ formatType + i);
							} else {
								formatSpan = $('<span></span>').attr('title',
										formatType + i).addClass(
										formatClass + ' ' + commonClass + ' '
												+ formatType + i);
							}
							var formatContainer = $('<span></span>').append(
									formatSpan).addClass(
									formatClass + '-container' + ' '
											+ 'container-span');
							$('<button></button>').append(formatContainer)
									.addClass(
											formatClass + '-button' + ' '
													+ commonClass + '-button')
									.appendTo($('.' + previewClass));
						}
					}
					...
					function validationError(args) {
						...
						if (validator.showErrorMessage()) {
							...
							if (validator.errorStyle() == 0 /* Stop */) {
								...
								designer.MessageBox.show(errorMessage,
										errorTitle, 3 /* error */,
										1 /* okCancel */, function(e,
												dialogResult) {
											switch (dialogResult) {
											case 1 /* ok */
											:
												// Simulate Retry
												args.sheet.setActiveCell(
														args.row, args.col);
												args.sheet.setValue(args.row,
														args.col, oldValue);
							...
							if (validator.errorStyle() == 1 /* Warning */) {
								...
								designer.MessageBox.show(errorMessage,
										errorTitle, 2 /* warning */,
										2 /* yesNoCancel */, function(e,
												dialogResult) {
											switch (dialogResult) {
											case 2 /* yes */
											:
					...
					function attachEvent(rebind) {
						...
						var isRibbonDirty;
						var isMergeDirty;
						var spread = designer.wrapper.spread, spreadElement = designer.wrapper.spreadElement;
						...
						spreadElement.bind(Sheets.Events.EnterCell,
								onSpreadEnterCell);
						...
						spread.bind(Sheets.Events.SelectionChanged,
								onSpreadSelectionChanged);
						spreadElement.bind(Sheets.Events.SelectionChanged,
								onSpreadSelectionChanged);

						spread.bind(Sheets.Events.ActiveSheetChanged, function(
								event, data) {
							updateShowHide();
							updateZoomToStatusBar();
						});
						spread
								.bind(
										Sheets.Events.EditorStatusChanged,
										function(event, data) {
											var editStatus = $(".statusBar")
													.find(
															".statusbar-edit-status");
											switch (data.newStatus) {
											case 2 /* Edit */
											:
												editStatus
														.text(designer.res.statusBar.edit);
												break;
											case 1 /* Enter */
											:
												editStatus
														.text(designer.res.statusBar.enter);
												break;
											case 0 /* Ready */
											:
												editStatus
														.text(designer.res.statusBar.ready);
												break;
											}
										});

						spread.bind(Sheets.Events.UserZooming, function(event,
								data) {
							updateZoomToStatusBar();
						});

						spread.bind(Sheets.Events.ValidationError, function(
								event, args) {
							validationError(args);
						});

						spread.bind(GC.Spread.Sheets.Events.SlicerChanged,
								function(event, args) {
									bindSlicerEvents(args.sheet, args.slicer,
											args.propertyName);
								});

						// only need bind once for other items except spread
						if (!ribbon.eventAttached) {
							$(designer).bind("updateRibbon",
									function(event, data) {
										updateCellStyle();
									});

							$(".designer-table-name").blur(
									function() {
										designer.actions.doAction(
												"setTableName",
												designer.wrapper.spread,
					...
					function _attachEvent() {
						// event
						var selectionChangedBindingPath = Sheets.Events.SelectionChanged
								+ ".bindingPath";
						var clickIndicator = "click.indicator";
						var clickOK = "click.ok";
						var clickCancel = "click.cancel";
						var topRowChangedBindingPath = Sheets.Events.TopRowChanged
								+ ".bindingPath";
						...
						var bindingPathSheet = designer.wrapper.spread
								.getActiveSheet();
						...
						dataFieldTypeButton
								.bind(
										...
										function(event, data) {
											...
											var dataFieldType = $(this).data(
													"datafieldtype");
											var container = $("#" + node.tId
													+ "_a");
											if (dataFieldType
													&& container.length > 0) {
												container
												...
												var specialTypeClass = dataFieldType
														+ "-icon";
												var typeStr = "<span class='button "
														+ buttonCommonClass
														+ " "
														+ specialTypeClass
														+ "' title='"
														+ designer.res.ribbon.data[dataFieldType]
														+ "'></span>";
												container.prepend(typeStr);
												if (dataFieldType === designer.util.DataFieldType.table
														|| dataFieldType === designer.util.DataFieldType.combox) {
													_refreshSchemaTreeView(
															"ztree-container",
															node.children);
												}
											}
										});
						tableBindingSpread.bind(selectionChangedBindingPath,
								function(event, data) {
						...
						indicator
								.bind(
										...
										function(event) {
											...
											var bindingPath = table
													.bindingPath();
											var ztreeObj = $.fn.zTree
													.getZTreeObj("ztree-container");
											var tableBindingPathArray = bindingPathInput
													.val().split(".");
											var treeNode = ztreeObj
													.getNodeByParam(
															"name",
															tableBindingPathArray[tableBindingPathArray.length - 1]);
											if (treeNode
													&& treeNode.children
													&& treeNode.dataFieldType === designer.util.DataFieldType.table) {
											...
											if (range) {
												...
												for (var i = 0; i < length; i++) {
													...
													newSelect
															.appendTo(dataFieldDiv);
												}
												documentFragment
														.appendTo(dataFieldContainer);
											}
										});
						okButton
								.bind(
										...
										function(event) {
											...
											var columns = [];
											for (var i = 0; i < length; i++) {
												var tableColumnInfo = new Sheets.Tables.TableColumn(
														i);
											...
											if (isAuto) {
												...
												var bindingPathInput = $(".table-binding-path-input");
												var datafields = _getDataField(bindingPathInput
														.val());
												var expectedColumnCount = datafields.length;
												if (expectedColumnCount !== tableColumnCount) {
													designer.MessageBox
															.show(
																	designer.res.ribbon.data.warningTable,
																	designer.res.title,
																	2 /* warning */,
																	1 /* okCancel */,
																	function(
						...
						bindingPathSheet.bind(topRowChangedBindingPath,
								function(event, data) {
									_refreshBindingPathIndicator();
								});
					...
					function _refreshBindingPathIndicator() {
						...
						if (table) {
							...
							if (bindingPath !== null
								...
								var cellRects = _getSpecifiedRect(new Sheets.Range(
										row, column, 1, 1));
								if (rects.length === 0) {
					...
					function _saveDataSource() {
						...
						var dialogInfo = {
							...
							nameFilters : [ designer.res.ribbon.data.saveDataSourceFilter.json ]
						};
						var callback = function(result) {
							if (result.status === 'cancelled') {
								return;
							...
							var zTree = $.fn.zTree
									.getZTreeObj("ztree-container");
							var zTreeRootNode = zTree.getNodesByParam("level",
									0)[0];
							var parent = [];
							...
							app.save(result.fileName, JSON
									.stringify(designer.util
											.saveJSONSchema(parent)),
									function(result) {
											...
											if (result.status === "failed") {
												...
												try {
													...
													if (data["$schema"]) {
														...
														_genZtreeNode(data,
																zTreeNode);
														originalDataSource = new Sheets.Bindings.CellBindingSource(
																data); // keep
																		// the
																		// real
																		// data
													...
													_initZTree(ztContainer,
															[ source ], true);
													_refreshSchemaTreeView(ztContainer);
												} catch (e) {
													designer.MessageBox
															.show(
																	designer.res.ribbon.data.loadDataError,
																	designer.res.title,
																	3 /* error */);
												}
											}
					...
					function _isNativeTreeViewJSON(jsonObj) {
						...
						if (mark["designerNativeSchemaMark"]
								&& mark["designerNativeSchemaMark"] === 1) {
							return true;
						}
						return false;
					}
					...
					function _refreshSchemaTreeView(container, nodes) {
						...
						_addMarker((nodes && nodes.length > 0) ? nodes
								: treeObj.getNodes());
					}
					...
					function _addMarkerCore(node) {
						...
						if (node.dataFieldType) {
							...
							dataFieldType = designer.util.DataFieldType.combox;
								...
								&& designer.util.DefaultTypeDic[node.type]) {
						...
						var typeStr = "<span class='button "
								+ buttonCommonClass + " " + specialTypeClass
								+ "' title='"
								+ designer.res.ribbon.data[dataFieldType]
								+ "'></span>";
					...
					function _replacePaint() {
						Sheets.CellTypes.Text.prototype["getText"] = function(
								value, context) {
							var bp = context.sheet.getBindingPath(context.row,
									context.col);
							if (bp) {
								return "[" + bp + "]";
							}
							return originalTextCellTypeGetTextLogic.apply(this,
									arguments);
						};
						Sheets.CellTypes.CheckBox.prototype["getText"] = function(
								value, options) {
							var bp = options.sheet.getBindingPath(options.row,
									options.col);
							if (bp) {
								return "[" + bp + "]";
							} else {
								return originalCheckboxCellTypeGetTextLogic
										.apply(this, arguments);
							}
						};
						Sheets.CellTypes.Button.prototype["getText"] = function(
								value, options) {
							var bp = options.sheet.getBindingPath(options.row,
									options.col);
							if (bp) {
							...
							return originalButtonCellTypeGetTextLogic.apply(
									this, arguments);
						};
						Sheets.CellTypes.HyperLink.prototype["getText"] = function(
								value, options) {
							var bp = options.sheet.getBindingPath(options.row,
									options.col);
							if (bp) {
								...
								return originalHyperlinkCellTypeGetTextLogic
										.apply(this, arguments);
							}
						};
						Sheets.CellTypes.ComboBox.prototype["getText"] = function(
								value, options) {
							var bp = options.sheet.getBindingPath(options.row,
									options.col);
							if (bp) {
								...
								return originalComboBoxCellTypeGetTextLogic
										.apply(this, arguments);
							}
						};
					}
						...
						var setting = {
							...
							callback : {
								...
								beforeDrop : function(treeId, treeNodes,
										targetNode, moveType) {
									return zTreeBeforeDrop(treeId, treeNodes,
											targetNode, moveType);
								},
								onDrop : function(event, treeId, treeNodes,
										targetNode, moveType) {
									zTreeOnDrop(event, treeId, treeNodes,
											targetNode, moveType);
								},
					...
					function zTreeBeforeRemove(treeId, treeNode) {
						if (treeNode && treeNode.children
							...
							designer.MessageBox
									.show(
											designer.res.ribbon.data.removeNodeWarning,
											designer.res.title,
											2 /* warning */,
											1 /* okCancel */,
					...
					function addHoverDom(container, treeNode) {
						...
						var addStr = "<span class='button add' id='addBtn_"
								+ treeNode.tId + "' title='"
								+ designer.res.ribbon.data.addNode
								+ "'></span>";
						sObj.after(addStr);
						...
						if (btn) {
							btn.bind("click", function() {
								...
								var newNode = zTree.addNodes(treeNode, [ {
									name : genNewNodeName(zTree, treeNode)
								} ]);
						...
						if ($("#arrayBtn_" + treeNode.tId).length <= 0
								&& treeNode.level !== 0) {
							var arrayStr = "<span class='button options' id='arrayBtn_"
									+ treeNode.tId
									+ "' title='"
									+ designer.res.ribbon.data.selectOptions
							...
							if (arrayBtn) {
								arrayBtn
										.bind(
												...
												function(event) {
													var zTreeObj = $.fn.zTree
															.getZTreeObj(container);
													zTreeObj
															.selectNode(treeNode);
													var buttons = $(".data-binding-celltype-button");
													buttons.button();
													var cellTypeSettingPopup = $(
															".data-binding-celltype-option")
															.gcuipopup(
																	{
																		autoHide : true,
													...
													cellTypeSettingPopup
															.gcuipopup("show");
													return false;
												});
							}
						}
						if (hasTableParent(treeNode)
								|| hasComboxParent(treeNode)) {
					...
					function genNewNodeName(zTreeObj, parent) {
						var name = "field-";
						var length = parent.children ? parent.children.length
								: 0;
						if (parent.level === 0) {
							name += length;
						} else {
					...
					function handleDocumentMouseMove() {
						var self = this;
						if (!isMouseCaptured) {
							$(document).bind("mousemove.bindingPath",
									function(event) {
										doMouseMove(event);
									}).bind("mouseup.bindingPath",
									function(event) {
										doMouseUp(event);
									});
					...
					function doMouseMove(event) {
						...
						var activeSheet = designer.wrapper.spread
								.getActiveSheet();
						...
						var rects = [], range;
						var table = activeSheet.tables.find(hitInfo.row,
								hitInfo.col);
						...
						if (table) {
							...
							if (parentNode
									&& getBindingPath(parentNode) === table
											.bindingPath()
									&& parentNode.dataFieldType === designer.util.DataFieldType.table) {
								// the data field
								range = new Sheets.Range(startRow, hitInfo.col,
							...
							if (hasTableParent(selectedNode)) {
								// To make datafield only can be set in table.
								// So, it shouldn't highlight in common cell
								// when drag a datafield.
								rects = [];
							} else if (hasComboxParent(selectedNode)
									&& activeSheet.getCellType(hitInfo.row,
											hitInfo.col).constructor !== Sheets.CellTypes.ComboBox) {
								...
								if (selections && selections.length > 0) {
									range = selections[0];
									if (range.contains(hitInfo.row,
											hitInfo.col, 1, 1)) {
										rects = _getSpecifiedRect(range);
									} else {
										range = new Sheets.Range(hitInfo.row,
												hitInfo.col, 1, 1);
										rects = _getSpecifiedRect(range);
									}
								}
					...
					function beginSetting(selectedNode, row, col) {
						...
						if (selectedNode.dataFieldType
								&& selectedNode.dataFieldType === designer.util.DataFieldType.table) {
							var bindingPathValue = getBindingPath(selectedNode);
							var datafields = _getDataField(bindingPathValue);
							if (table) {
								// set binding path for table
								_setBindingPath(table, bindingPathValue,
										datafields);
							} else {
								// generate table
								if (selection.contains(row, col, 1, 1)) {
									_addTableByArray(selection,
											bindingPathValue, datafields);
								} else {
									var newSelection = new Sheets.Range(row,
											col, 1, 1);
									_addTableByArray(newSelection,
											bindingPathValue, datafields);
								}
							}
						} else if (table
							...
							_bindTableColumns(col - table.range().col,
									arr[arr.length - 1], table);
						} else if (currentCellType
								&& currentCellType.constructor === Sheets.CellTypes.ComboBox
								&& parentNode
								&& parentNode.dataFieldType === designer.util.DataFieldType.combox) {
							...
							if (hasTableParent(selectedNode)) {
								designer.MessageBox
										.show(
												designer.res.ribbon.data.unallowableTableBindingTip,
												designer.res.title,
												2 /* warning */, 0 /* ok */);
								return;
							}
							if (hasComboxParent(selectedNode)) {
								designer.MessageBox
										.show(
												designer.res.ribbon.data.unallowComboxBindingTip,
												designer.res.title,
												2 /* warning */, 0 /* ok */);
								return;
							}
					...
					function preSetting(selectedNode, startRow, startColumn,
							callback) {
						// whether to overwrite some celltype
						if (!selectedNode || isNaN(startRow)
								|| isNaN(startColumn)) {
							return;
						}
						var ztreeObj = $.fn.zTree
						...
						var activeSheet = designer.wrapper.spread
								.getActiveSheet();
						var leafNodes = ztreeObj.getNodesByFilter(isCommonNode,
								false, selectedNode);
						var delta = 0;
						...
						var endColumn = isAutoGenerateLabel ? startColumn
								+ delta + 1 : startColumn + delta;
						...
						if (isSuccess) {
							...
							designer.MessageBox
									.show(
											designer.res.ribbon.data.overwriteCellTypeWarning,
											designer.res.title,
											2 /* warning */,
											1 /* okCancel */, function(event,
													result) {
												if (result === 1 /* ok */) {
													callback();
												}
					...
					function hasTableParent(node) {
						...
						while (tempNode
								&& tempNode.dataFieldType !== designer.util.DataFieldType.table
								&& tempNode.level !== 0) {
					...
					function hasComboxParent(node) {
						...
						while (tempNode
								&& tempNode.dataFieldType !== designer.util.DataFieldType.combox
								&& tempNode.level !== 0) {
							tempNode = tempNode.getParentNode();
						}
						if (tempNode && tempNode.level === 0) {
					...
					function setDetails(treeNode) {
						...
						if (treeNode.dataFieldType === designer.util.DataFieldType.table) {
							...
							var newSelection = new Sheets.Range(
									detailsPosition.row, detailsPosition.col
											+ delta, 1, 1);
							var result = _addTableByArray(newSelection,
									bindingPath, datafields);
							if (result) {
								detailsPosition.row += 2; // default table
															// column count is
															// 2.
							}
							return;
							...
							var comboxItems = treeNode["enum"] ? treeNode["enum"]
									: _getDataField(getBindingPath(treeNode));
							if (comboxItems && comboxItems.length > 0) {
								comboBoxCellType.items(comboxItems);
							}
							if (isAutoGenerateLabel) {
								activeSheet.setValue(detailsPosition.row,
										detailsPosition.col + delta,
										treeNode.name);
								activeSheet.getCell(detailsPosition.row,
										detailsPosition.col + delta + 1)
										.bindingPath(bindingPath);
								activeSheet.setCellType(detailsPosition.row,
										detailsPosition.col + delta + 1,
										comboBoxCellType);
							} else {
								activeSheet.getCell(detailsPosition.row,
						...
						if (subNodes && subNodes.length > 0) {
							...
							if (isAutoGenerateLabel) {
								...
								activeSheet.setValue(row, col - 1,
										treeNode.name);
								activeSheet.getCell(row, col).bindingPath(
										bindingPath);
							} else {
								row = detailsPosition.row;
					...
					function getHitTestInfo(event) {
						...
						var t = $(
								designer.util
										.getCanvas(designer.wrapper.spread))
								.offset();
						return activeSheet.hitTest(event.pageX - t.left,
								event.pageY - t.top);
					}
					...
					function zTreeBeforeDrop(treeId, treeNodes, targetNode,
						...
						if (!isCommonNode(targetNode)) {
							// Can't add node to the children of table or
							// combox.
							return false;
						}
						var nodeType = targetNode.dataFieldType;
						var DataFieldType = designer.util.DataFieldType;
						if ((nodeType === DataFieldType.table || nodeType === DataFieldType.combox)
								&& treeNodes[0].children
								&& treeNodes[0].children.length > 0) {
							// Don't allow node with children is added to table
							// or combox.
							return false;
						}
						return true;
					...
					function treeNodeToArray(treeNodes, parent) {
						...
						for (var i = 0; i < length; i++) {
							...
							if (treeNodes[i].children
								...
								node["children"] = [];
								treeNodeToArray(treeNodes[i].children,
										node["children"]);
							}
							parent.push(node);
						}
					...
					function _getSpecifiedRect(range) {
						...
						for (var r = 0; r <= 2; r++) {
							for (var c = 0; c <= 2; c++) {
								...
								var rightColumn = sheet
										.getViewportRightColumn(c);
								var topRow = sheet.getViewportTopRow(r);
								var bottomRow = sheet.getViewportBottomRow(r);
								var ra = new Sheets.Range(topRow, leftColumn,
										bottomRow - topRow + 1, rightColumn
												- leftColumn + 1);
								var intersect = range.getIntersect(ra, Math
										.max(ra.rowCount, range.rowCount), Math
										.max(ra.colCount, range.colCount));
								if (intersect) {
									var rect = new Sheets.Rect();
									var tempRect = sheet.getRangeRect(r, c,
											intersect);
					...
					function _bindTableColumns(index, dataField, table) {
						...
						if (isAuto) {
							designer.MessageBox.show(
									designer.res.ribbon.data.warningDataField,
									designer.res.title, 2 /* warning */,
									1 /* okCancel */, function(event, result) {
										if (result === 4 /* cancel */) {
											return;
										} else {
											table.autoGenerateColumns(false);
											table.setColumnName(index,
													dataField);
											table.setColumnDataField(index,
													dataField);
											sheet.repaint();
										}
									});
					...
					function _highlightBlock(rects) {
						...
						for (var i = 0; i < rects.length; i++) {
							var decoration = $("<div>").addClass(
									"data-binding-decoration").appendTo(
									fragment);
							var rect = rects[i];
					...
					function _getDataField(parent) {
						...
						var treeNode = ztreeObj.getNodeByParam("name",
								parentStr);
						if (treeNode && treeNode.children) {
					...
					function _setBindingPath(table, bindingPath, datafield) {
						// When setting binding path on table, need to set data
						...
						var tableColumnCount = table.endColumn()
								- table.startColumn() + 1;
						var length = Math
								.max(dataFieldLength, tableColumnCount);
						var columns = [];
						table.bindColumns(columns);
						if (length > 0) {
							for (var i = 0; i < length; i++) {
					...
					function _addTableByArray(selection, bindingPath, datafield) {
						...
						try {
							if (isAutoGenerateLabel) {
								...
								table = sheet.tables.add(_getTableName(), row,
										col + 1, rowCount, colCount);
							} else {
								table = sheet.tables.add(_getTableName(), row,
							...
							sheet.setSelection(row,
									isAutoGenerateLabel ? col + 1 : col,
									rowCount, colCount);
						} catch (e) {
							designer.MessageBox
									.show(e.message, designer.res.title,
											3 /* error */, 0 /* ok */);
							return;
						}
						return table;
					...
					function initSliderPanel() {
						...
						var sliderPanelContent = sliderPanel
								.sliderpanel("getContent");
						var ztreeContainer = $("#" + ztreeID);
						if (ztreeContainer.length <= 0) {
							$("<div>").attr("id", ztreeID).addClass("ztree")
									.appendTo(sliderPanelContent);
						}
						...
						if (designer.wrapper.spread["designerBindingPathSchema"]
								&& !isModified) {
							var data = designer.wrapper.spread["designerBindingPathSchema"];
							if (data["$schema"]) {
								// load standard cell binding template json
								// schema.
								bindingPathData["children"] = designer.util
										.parseJsonSchema(data);
								_initZTree("ztree-container",
										[ bindingPathData ], true);
							} else if ($.isArray(data)
									&& _isNativeTreeViewJSON(data)) {
								// load old version cell binding template json
								// schema.
					...
					function insertTableSlicer(table, columnsChecked) {
						...
						for (var i = 0; i < columnsChecked.length; i++) {
							var columnName = table
									.getColumnName(columnsChecked[i]);
							var slicerName = getAutoSlicerName(spread,
									columnName);
							var slicer = sheet.slicers.add(slicerName, table
									.name(), columnName);
							slicer.position(new GC.Spread.Sheets.Point(posX,
									posY));
							posX = posX + 30;
							posY = posY + 30;
						}
					...
					function showSlicerTab() {
						...
						var $slicerButtonDiv = $("#slicerTab>ul").find(
								"li:eq(2)");
						var columnsButtonWidth = $slicerButtonDiv.find(
								"button:eq(0)").width();
						var heightButtonWidth = $slicerButtonDiv.find(
								"button:eq(1)").width();
						var widthButtonWidth = $slicerButtonDiv.find(
								"button:eq(1)").width();
						var maxButtonDivNumber = (columnsButtonWidth > heightButtonWidth) ? columnsButtonWidth
								: heightButtonWidth;
						maxButtonDivNumber = (maxButtonDivNumber > widthButtonWidth) ? maxButtonDivNumber
								: widthButtonWidth;

						var $slicerSizeDiv = $("#slicerTab>ul")
								.find("li:eq(3)");
						var shapeHeightButtonWidth = $slicerSizeDiv.find(
								"button:eq(0)").width();
						var shapeWidthButtonWidth = $slicerSizeDiv.find(
								"button:eq(1)").width();
						var maxSizeDivNumber = (shapeHeightButtonWidth > shapeWidthButtonWidth) ? shapeHeightButtonWidth
								: shapeWidthButtonWidth;
						$("#slicerTab>ul").find("li:eq(2)").find("button")
								.width(maxButtonDivNumber);
						$("#slicerTab>ul").find("li:eq(3)").find("button")
								.width(maxSizeDivNumber);
						$("#slicerTab>ul").find("li:gt(1)").find("button")
								.find("span:gt(0)").css("text-align", "start");
					}
					...
					function initSlicerTab(slicer) {
						...
						$("#slicer-shape-width")
								.val(Math.round(slicer.width()));
						$("#slicer-shape-height").val(
								Math.round(slicer.height()));
						$("#slicer-column-count").val(slicer.columnCount());
						$("#slicer-item-height").val(
								Math.round(slicer.itemHeight()));
						var item_width = designer.util.getSlicerItemWidth(
								slicer.columnCount(), slicer.width());
						$("#slicer-item-width").val(item_width);
						showSlicerTab();

						bindInputEvent("caption-name",
								$(".slicer-caption-name"));
						bindInputEvent("shape-width", $("#slicer-shape-width"));
						bindInputEvent("shape-height",
								$("#slicer-shape-height"));
						bindInputEvent("column-count",
								$("#slicer-column-count"));
					...
					function bindSlicerEvents(sheet, slicer, propertyName) {
						...
						if (propertyName === "isSelected") {
							if (slicer.isSelected()) {
								if (sheet.options.protectionOptions.allowEditObjects
										|| !(sheet.options.isProtected && slicer
												.isLocked())) {
									initSlicerTab(slicer);
					...
					function setSlicerProperty(property, selectedSlicers, value) {
						...
						case "item-width":
							...
							for ( var item in selectedSlicers) {
								...
								var width = designer.util
										.getSlicerWidthFromItem(slicer
					...
					function changeSlicerInfo(slicer, propertyName) {
						...
						case "width":
							...
							$("#slicer-item-width").val(
									designer.util.getSlicerItemWidth(slicer
											.columnCount(), slicer.width()));
							break;
						case "height":
					...
					function changeSlicerStyle(sheet, value) {
						...
						switch (value) {
						case "light1":
							slicerStyle = GC.Spread.Sheets.Slicers.SlicerStyles
									.light1();
							break;
						case "light2":
							slicerStyle = GC.Spread.Sheets.Slicers.SlicerStyles
					...
					function _initSlicerFormat() {
						...
						_createSlicerFormat('slicer-custom-preview', 'custom',
								customCount);
						_createSlicerFormat('slicer-light-preview', 'light',
								lightCount);
						_createSlicerFormat('slicer-dark-preview', 'dark',
								darkCount);

						$(
								'.slicer-format-preview, .slicer-format-preview-button')
								.mouseenter(
										function(e) {
											var o = (e.srcElement || e.target);
											$(o).closest("button").find(
													...
													var customPrefix = name
															.substring(
																	...
																	name
																			.indexOf('-'));
													if (customPrefix === style) {
														_setSelectedSlicersStyle(
																sheet,
																designer.SlicerStyleDialog.customSlicerStyle[name]);
													}
												}
											}
											$("#format-slicer-popup")
					...
					function _createSlicerFormat(previewClass, formatType,
							formatTypeNumber) {
						var slicerFormatClassPrefix = 'slicer-format-';
						var commonClass = 'slicer-format-preview';
						var formatSpan;
						for (var i = 1; i <= formatTypeNumber; i++) {
							var formatClass = slicerFormatClassPrefix
									+ formatType + i;
							if (formatType === 'custom') {
								formatSpan = $('<span></span>').addClass(
										formatClass + ' ' + commonClass + ' '
								...
								var value = i - 6;
								formatClass = slicerFormatClassPrefix
										+ slicerType + value;
								formatSpan = $('<span></span>').attr('title',
										slicerType + value).addClass(
										formatClass + ' ' + commonClass + ' '
												+ slicerType + value);
							} else {
								formatSpan = $('<span></span>').attr('title',
					...
					function addLocalFonts() {
						var res = designer.res, name = res.name || 'en', fontFamilies = res.ribbon.fontFamilies, prefix = name
								+ 'ff', fontName = fontFamilies[prefix + '1'];
						if (fontName) {
							var $fontFamily = $("#font-family"), $ul = $fontFamily
									.next('ul');
