    ...
    btnEdit = function () {
        ...
        this.initControl = function (props) {
            ...
            if (type === 'singletree' || type === "checktree") {
                ...
                ctl.push($(ipt).attr('class', 'btn-edit')[0].outerHTML);
                ctl.push('</span>');
            ...
            $con.find('.btn-edit-icon, .btn-edit-text').bind('click', function () {
                ...
                /*ipt_pos.x = $(this).offset().top;
                ipt_pos.y = $(this).offset().left;
                var dlg_style = $con.find('.btn-edit-dialog').attr('style');
                var new_top = ipt_pos.x - 610;  // 610是窗口高度
                $con.find('.btn-edit-dialog').attr('style', dlg_style + ';top:' + new_top + 'px;');
                console.log(ipt_pos);*/
            });
            $con.find('.dlg-cancel, .dlg-ok, .btn-edit-dialog-title>span').on('click', function () {
                ...
                success: function (data) {
                    ...
                    if (type === 'singletree') {
                        $con.find(tree_class).jstree({
                            ...
                            result.push({key: node.id, value: node.text, level: node.level || -1, child_key: '', child_value: '', child_level: -1 });
                            if (props.right_cart) {                                        
                                    ...
                                    if (props.right_cart) {                                        
                                        ...
                                        if (has_level) {
                                            $con.find('.btn-edit-right-dialog')
                                                    .append("<li key='" + node.id + "'><a href='javascript:;'><span style='background: url("+node_icon+")'></span>" 
